...
var XDashFusionChartComponentAsync = UnmanagedComponent.extend({
	 ...
	 render: function(values) {
		...
		if(!_.has(cd, 'chartProperties') || !_.has(cd, 'dataSetProperties') || !_.has(cd, 'connectorsProperties')){
			//display mission properties error
			$("#"+myself.htmlObject).html(
					"<div class=\"ui-state-error ui-corner-all\" style=\"padding: 0 .7em;\">"+
					"<p style=\"margin: 0 0 .3em;\">"+
					"<span class=\"ui-icon ui-icon-alert\" style=\"float: left; margin-right: .3em;\"></span>"+
					"<strong>Error:</strong>Missing properties (chartProperties, dataSetProperties or connectorProperties)</p></div>");
		} else if (!_.has(cd, 'chartType') || !_.has(myself, 'htmlObject') || !_.has(cd, 'width') || !_.has(cd, 'height')) {
			...
			var queryConnectors = values.resultset.map(function(i) { return (i[4]!=null?{"strength": i[6], "from": i[2], "to": i[4],"label": i[5]} : undefined); });
			...
			var nodes = [];
			var uniqueVals = [];
			$.each(queryDataset, function(i, el){
				if($.inArray(el["id"], uniqueVals) === -1) {uniqueVals.push(el["id"]); nodes.push(el);}
			});

			//apply node callback function
			if(_.has(cd.dataSetProperties, 'nodeCallback')){
				nodes = nodes.map(function(i){
					cd.dataSetProperties.nodeCallback(i);
					return i
				});
			};

			//apply connectors callback function
			if(_.has(cd.connectorsProperties, 'connectorCallback')){
				queryConnectors = queryConnectors.map(function(i){
					cd.connectorsProperties.connectorCallback(i);
					return i;
				});
			}

			// add nodes data to dataset
			cd.dataSetProperties.data = nodes;
			...
			if(_.has(cd, 'trendlinesProperties')){
				if(_.has(cd.trendlinesProperties, 'dataAccessId') && _.has(cd.trendlinesProperties, 'path')){
					...
					if(_.has(cd.trendlinesProperties, 'lineCallback')){
						resultset = resultset.map(function(i){
							cd.trendlinesProperties.lineCallback(i);
							return i
						});
					};
			...
			if(_.has(cd, 'labelsProperties')){
				if(_.has(cd.labelsProperties, 'dataAccessId') && _.has(cd.labelsProperties, 'path')){
					...
					if(_.has(cd.labelsProperties, 'labelCallback')){
						resultset = resultset.map(function(i){
							cd.labelsProperties.labelCallback(i);
							return i
						});
					};
